...
function convert($filename, $save) {
...
function traverseDir($dir, $save)
{
    if (strpos($dir, 'Exception') !== false) {
        return;
    }
    ...
    while (false !== ($entry = readdir($handle))) {
        ...
        if (is_dir($entryPath)) {
            traverseDir($entryPath, $save);
            continue;
        } elseif (strpos($entryPath, '.php') === false) {
            continue;
        }

        convert($entryPath, $save);
    }
}
...
if (is_dir($filePath)) {
    ...
    while (false !== ($entry = readdir($root))) {
        ...
        if (!is_dir($entryPath) || substr($entry, 0, 1) == '.') {
            continue;
        }

        traverseDir($entryPath, $saveFile);
    }
} else {
